...
export default function ResultsPage() {
    ...
    if (!test) return (
        <div>
            ...
            <div style={{ textAlign: 'center', padding: '80px 24px', color: 'var(--muted)' }}>
                ...
                <p>Test not found.</p>
                <Link href="/student/dashboard" style={{ color: '#a78bfa' }}>Back to Dashboard</Link>
            </div>
        </div>
    );
    ...
    const scoreLabel = pct >= 80 ? 'Excellent! 🎉' : pct >= 60 ? 'Good job! 👍' : 'Keep practicing! 💪';
            ...
            <main style={{ maxWidth: '820px', margin: '0 auto', padding: '32px 24px' }}>
                ...
                <div className="glass fade-in" style={{ padding: '40px', textAlign: 'center', marginBottom: '32px', position: 'relative', overflow: 'hidden' }}>
                    ...
                    <p style={{ color: 'var(--muted)' }}>
                        {test.score}/{test.total_questions} correct · {test.subject_name} · {test.difficulty} difficulty
                    </p>
                    <div style={{ display: 'flex', justifyContent: 'center', gap: '16px', marginTop: '24px' }}>
                        <button className="btn-primary" onClick={() => router.push('/student/test')}>Take Another Test</button>
                        <button className="btn-secondary" onClick={() => router.push('/student/dashboard')}>Dashboard</button>
                    </div>
                </div>

                {/* Question breakdown */}
                <div className="glass" style={{ padding: '24px', marginBottom: '24px' }}>
                    <h2 style={{ fontSize: '1.1rem', fontWeight: '700', marginBottom: '20px' }}>📋 Question Breakdown</h2>
                                ...
                                <div key={i} style={{
                                    ...
                                    <button
                                        ...
                                        style={{
                                            ...
                                            padding: '16px 20px', cursor: 'pointer', display: 'flex', alignItems: 'flex-start', gap: '12px',
                                        }}>
                                        <span style={{ fontSize: '1.2rem', flexShrink: 0 }}>{correct ? '✅' : '❌'}</span>
                                        <div style={{ flex: 1 }}>
                                            <p style={{ color: 'var(--text)', fontSize: '0.9rem', fontWeight: '500', lineHeight: 1.5 }}>
                                                {i + 1}. {q.question_text}
                                            </p>
                                        ...
                                        <div style={{ padding: '0 20px 20px', borderTop: '1px solid var(--border)' }}>
                                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginTop: '16px' }}>
                                                <div style={{ padding: '12px', borderRadius: '8px', background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.2)' }}>
                                                    <p style={{ color: '#f87171', fontSize: '0.75rem', fontWeight: '700', marginBottom: '4px' }}>YOUR ANSWER</p>
                                                    <p style={{ color: 'var(--text)', fontSize: '0.9rem' }}>{q.student_answer || 'No answer'}</p>
                                                </div>
                                                <div style={{ padding: '12px', borderRadius: '8px', background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.2)' }}>
                                                    <p style={{ color: '#34d399', fontSize: '0.75rem', fontWeight: '700', marginBottom: '4px' }}>CORRECT ANSWER</p>
                                            ...
                                            {q.explanation && (
                                                <div style={{ marginTop: '12px', padding: '14px', borderRadius: '8px', background: 'rgba(124,58,237,0.08)', border: '1px solid rgba(124,58,237,0.2)' }}>
                                                    <p style={{ color: '#a78bfa', fontSize: '0.75rem', fontWeight: '700', marginBottom: '4px' }}>💡 EXPLANATION</p>
                ...
                {test.recommendations?.length > 0 && (
                    <div className="glass" style={{ padding: '24px' }}>
                        <h2 style={{ fontSize: '1.1rem', fontWeight: '700', marginBottom: '16px' }}>💡 AI Study Recommendations</h2>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
